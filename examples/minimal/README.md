...
The smallest valid call to this module: one internal load balancer on a fresh vnet, with a
zone-redundant dynamic frontend, a backend pool, a Tcp health probe, and a load-balancing rule.
The environment comes from the Terraform workspace (`terraform.workspace`), not a variable. Run it
